...
const scannerLocations: number[][] = [[0, 0, 0]]
        ...
        for (let beacon of rotation) {
            for (let [fx, fy, fz] of foundList) {
                ...
                if (matches >= 12) {
                    ...
                    scannerLocations.push([dx, dy, dz])
...
let longest = 0
for (let [x, y, z] of scannerLocations) {
    for (let [a, b, c] of scannerLocations) {
        const dist = Math.abs(x - a) + Math.abs(y - b) + Math.abs(z - c)
        if (dist > longest) longest = dist
    }
}

console.log('Number of scanners found:', foundList.length)
console.log('Max distance:', longest)
