...
const boards: board[] = boardsFile.split('\n\n').map((board) =>
    board.split('\n').map((line) =>
        line
            .trim()
            .split(/\s+/)
...
callingLoop: for (let i = 1; i < numbers.length; i++) {
    ...
    for (let board of loserBoards) {
        if (isSolved(called, board)) {
            if (loserBoards.length == 1) {
                solutionFound(called, loserBoards[0])
                break callingLoop
            }
            loserBoards = loserBoards.filter((lboard) => lboard != board)
        }
    }
}
