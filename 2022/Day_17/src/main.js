...
function dfs(currentValve, openedValves, time) {
    ...
    const key = `${currentValve}-${time}-${openedValves.sort((a, b) => a.charCodeAt(0) - b.charCodeAt(0)).join('')}`
    ...
    if (!openedValves.includes(currentValve) && map[currentValve].rate > 0) {
        totalPressure = Math.max(
            totalPressure,
            map[currentValve].rate * (time - 1) + dfs(currentValve, [...openedValves, currentValve], time - 1),
        )
    }

    for (const nextTunnel of map[currentValve].nextTunnels) {
        totalPressure = Math.max(totalPressure, dfs(nextTunnel, openedValves, time - 1))
    }
